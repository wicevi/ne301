...
#else
static UINT txfr_idle_stack[configMINIMAL_STACK_SIZE] __attribute__ ((section (".psram_bss")));
#endif
    ...
    for(;;) {
        ...
        if(p_task != NULL) {
            ...
            if(p_task->allocated == 1u) {
                if (p_task->p_stack != NULL) {
                    txfr_free(p_task->p_stack);
                }
                txfr_free(p_task);
            }
        }
        // printf("idle delete end: %p\n", p_task);
    }
}
#endif // #if (INCLUDE_vTaskDelete == 1)
...
void vTaskDelete(TaskHandle_t xTask)
{
    ...
    if (xTask == NULL) {
        ...
        // printf("delete self end: %p\n", p_txfr_thread);
    } else {
        p_txfr_thread = xTask;
        p_thread = &xTask->thread;
        // printf("other delete start: %p\n", p_txfr_thread);

        (void)tx_thread_terminate(p_thread);

        ret = tx_thread_delete(p_thread);
        if(ret != TX_SUCCESS) {
            TX_FREERTOS_ASSERT_FAIL();
        }

        ret = tx_semaphore_delete(&p_txfr_thread->notification_sem);
        if(ret != TX_SUCCESS) {
            TX_FREERTOS_ASSERT_FAIL();
        }

        if (p_txfr_thread->allocated == 1u) {
            if (p_txfr_thread->p_stack != NULL) {
                txfr_free(p_txfr_thread->p_stack);
            }
            txfr_free(p_txfr_thread);
        }

        // printf("other delete end: %p\n", p_txfr_thread);
    }
    return;
}
#endif
