...
nau881x_status_t NAU881x_Set_PGA_Gain_db(NAU881x_t* nau881x, float vol_db)
{
    ...
    uint8_t val = (uint8_t)((vol_db + 12.0f) / 0.75f);
...
/* Clear PUDOEN so ADCOUT drives actual ADC data instead of mask/tie-off.
 * Datasheet: PUDOEN=1 switches output to "mask"; we need PUDOEN=0 for normal I2S. */
nau881x_status_t NAU881x_Set_ADCOUT_DriveNormal(NAU881x_t* nau881x)
{
    uint16_t regval = nau881x->_register[NAU881X_REG_ADCOUT_DRIVE];
    regval &= ~((1 << 5) | (1 << 4) | (1 << 3)); /* PUDOEN, PUDPE, PUDPS = 0 */
    return NAU881x_Register_Write(nau881x, NAU881X_REG_ADCOUT_DRIVE, regval);
}
...
{
    nau881x->_register[NAU881X_REG_SILICON_REV] = NAU881X_REG_READ(nau881x, NAU881X_REG_SILICON_REV);
...
nau881x_status_t NAU881x_Register_Write(NAU881x_t* nau881x, uint8_t register_addr, uint16_t value)
{
    NAU881X_REG_WRITE(nau881x, register_addr, value);
