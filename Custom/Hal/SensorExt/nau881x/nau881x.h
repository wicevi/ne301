...
/**
 * Function pointer types for platform I2C access.
 * write_reg: returns 0 on success, negative on error.
 * read_reg:  returns the 9-bit register value, or 0 on error.
 */
typedef int      (*nau881x_write_reg_fn)(uint16_t dev_addr, uint8_t reg, uint16_t value);
typedef uint16_t (*nau881x_read_reg_fn) (uint16_t dev_addr, uint8_t reg);

typedef struct _NAU881x
{
    uint32_t comm_handle;       /* 8-bit I2C device address (7-bit << 1) */
    nau881x_write_reg_fn write_reg;
    nau881x_read_reg_fn  read_reg;
    uint16_t _register[80];
} NAU881x_t;

/* Internal helpers — must appear after NAU881x_t is fully defined. */
static inline void _nau881x_reg_write(NAU881x_t *h, uint8_t reg, uint16_t val)
{
    if (h->write_reg) h->write_reg((uint16_t)h->comm_handle, reg, val);
}
static inline uint16_t _nau881x_reg_read(NAU881x_t *h, uint8_t reg)
{
    return h->read_reg ? h->read_reg((uint16_t)h->comm_handle, reg) : 0;
}

#define NAU881X_REG_WRITE(handle, reg, val) _nau881x_reg_write(handle, reg, val)
#define NAU881X_REG_READ(handle, reg)       _nau881x_reg_read(handle, reg)
...
/* Set digital filter sample rate (SMPLR, Reg 0x07): 0=48k,1=32k,2=24k,3=16k,4=12k,5=8k */
nau881x_status_t NAU881x_Set_SampleRate(NAU881x_t* nau881x, uint8_t smplr);
nau881x_status_t NAU881x_Set_LOUTR(NAU881x_t* nau881x, uint8_t enable);
nau881x_status_t NAU881x_Set_ADCOUT_DriveNormal(NAU881x_t* nau881x);
