...
#ifdef CONFIG_IEEE80211AH
	if (bss->max_away_duration > conf->beacon_int) {
		wpa_printf(MSG_ERROR,
			   "Max Away Duration cannot exceed Beacon Interval: %d > %d",
			   bss->max_away_duration, conf->beacon_int);
		return -1;
	}
#endif
