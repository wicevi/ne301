...
/* Per-instance parameters for BlazeFace UU postprocess */
typedef struct {
...
{
    pp_od_fd_blazeface_uu_params_t *pp_ctx = (pp_od_fd_blazeface_uu_params_t *)hal_mem_alloc_any(sizeof(pp_od_fd_blazeface_uu_params_t));
    if (!pp_ctx) {
...
static int32_t run(void *pInput[], uint32_t nb_input, void *pResult, void *pp_params, void *nn_inst)
{
    ...
    // BlazeFace expects: scores_0, scores_1, detections_0, detections_1 (uint8 for UU version)
    ...
    error = od_fd_blazeface_pp_process_uint8(&pp_input, &od_pp_out, params);
...
const pp_entry_t pp_entry_od_fd_blazeface_uu = {
    .name = "pp_od_fd_blazeface_uu",
    .vt = &vt
};
