...
/* Per-instance parameters for BlazeFace UI postprocess (treated as OD) */
typedef struct {
...
/*
JSON fields are the same as UF variant; additionally, the model is int8-quantized.
*/
static int32_t init(const char *json_str, void **pp_params, void *nn_inst)
{
    pp_od_fd_blazeface_ui_params_t *pp_ctx =
        (pp_od_fd_blazeface_ui_params_t *)hal_mem_alloc_any(sizeof(pp_od_fd_blazeface_ui_params_t));
    if (!pp_ctx) {
    ...
    /* Quantization information from NN instance (4 outputs) */
    ...
    /* Default values similar to UF variant */
    params->nb_classes      = 1;
...
static int32_t run(void *pInput[], uint32_t nb_input, void *pResult, void *pp_params, void *nn_inst)
{
    ...
    error = od_blazeface_pp_process_int8(&pp_input, &od_pp_out, params);
