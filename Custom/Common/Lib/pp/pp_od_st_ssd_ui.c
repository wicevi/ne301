...
/* Per-instance parameters for SSD UI postprocess (int8) */
typedef struct {
    ...
    od_pp_outBuffer_t *scratch_buffer; /* per-instance scratch buffer */
    float32_t *scratch_softmax;        /* per-instance softmax scratch buffer */
    od_detect_t *od_detect_buffer;     /* per-instance detection buffer */
    char **class_names;                /* per-instance class name array */
} pp_od_st_ssd_ui_params_t;

/*
JSON fields are the same as UF variant; additionally, quant parameters come from NN instance.
*/
static int32_t init(const char *json_str, void **pp_params, void *nn_inst)
{
    pp_od_st_ssd_ui_params_t *pp_ctx =
        (pp_od_st_ssd_ui_params_t *)hal_mem_alloc_any(sizeof(pp_od_st_ssd_ui_params_t));
    if (!pp_ctx) {
        return AI_OD_POSTPROCESS_ERROR_NO;
    }
    memset(pp_ctx, 0, sizeof(pp_od_st_ssd_ui_params_t));

    od_ssd_pp_static_param_t *params = &pp_ctx->core;

    /* Defaults (aligned with ST macros, can be overridden by JSON) */
    params->nb_classes      = 80;
    params->nb_detections   = 1917;
    params->XY_inv_scale    = 0.1f;
    params->WH_inv_scale    = 0.2f;
    ...
    params->pScratchBufferSoftMax = NULL;

    /* Quantization parameters from NN instance (2 outputs: boxes, scores) */
    ...
    if (NN_Instance != NULL) {
        ...
        if (buffers_info != NULL && buffers_info[0].scale != NULL) {
            params->boxe_scale      = *(buffers_info[0].scale);
            params->boxe_zero_point = (int8_t)(*buffers_info[0].offset);
        }
        if (buffers_info != NULL && buffers_info[1].scale != NULL) {
            params->score_scale      = *(buffers_info[1].scale);
            params->score_zero_point = (int8_t)(*buffers_info[1].offset);
        }
    }
    ...
    size_t boxes_limit = MAX((size_t)params->max_boxes_limit, (size_t)params->nb_detections);
    pp_ctx->od_pp_buffer =
        (od_pp_outBuffer_t *)hal_mem_alloc_large(sizeof(od_pp_outBuffer_t) * boxes_limit);
    pp_ctx->scratch_buffer =
        (od_pp_outBuffer_t *)hal_mem_alloc_large(sizeof(od_pp_outBuffer_t) * params->nb_detections);
    pp_ctx->scratch_softmax =
        (float32_t *)hal_mem_alloc_any(sizeof(float32_t) * params->nb_classes);
    ...
    assert(pp_ctx->od_pp_buffer != NULL &&
           pp_ctx->scratch_buffer != NULL &&
           pp_ctx->scratch_softmax != NULL &&
           pp_ctx->od_detect_buffer != NULL);

    params->pScratchBuffer = pp_ctx->scratch_buffer;
    params->pScratchBufferSoftMax = pp_ctx->scratch_softmax;
...
{
    pp_od_st_ssd_ui_params_t *pp = (pp_od_st_ssd_ui_params_t *)pp_params;
    if (!pp) {
        return AI_OD_POSTPROCESS_ERROR_NO;
    }
    ...
    if (pp->scratch_softmax != NULL) {
        hal_mem_free(pp->scratch_softmax);
        pp->scratch_softmax = NULL;
    }
...
static void od_pp_out_t_to_pp_result_t(od_pp_out_t *pObjDetOutput,
                                       pp_result_t *result,
                                       const pp_od_st_ssd_ui_params_t *pp)
{
...
static int32_t run(void *pInput[], uint32_t nb_input, void *pResult, void *pp_params, void *nn_inst)
{
    ...
    error = od_ssd_pp_process_int8(&pp_input, &od_pp_out, params);
...
{
    pp_od_st_ssd_ui_params_t *pp = (pp_od_st_ssd_ui_params_t *)params;
