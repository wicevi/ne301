...
/* Per-instance parameters for YOLOv5 UU postprocess */
typedef struct {
    od_yolov5_pp_static_param_t core;     /* original static params, now per-instance */
...
{
    pp_od_yolo_v5_uu_params_t *pp_ctx = (pp_od_yolo_v5_uu_params_t *)hal_mem_alloc_any(sizeof(pp_od_yolo_v5_uu_params_t));
    if (!pp_ctx) {
        return AI_OD_POSTPROCESS_ERROR_NO;
    }
    memset(pp_ctx, 0, sizeof(pp_od_yolo_v5_uu_params_t));

    od_yolov5_pp_static_param_t *params = &pp_ctx->core;
    
    // Get quantization parameters from NN instance (for uint8 models)
        ...
        if (root != NULL) {
            ...
            if (cJSON_IsObject(pp)) {
                ...
                if (cJSON_IsNumber(raw_zero)) {
                    params->raw_output_zero_point = (uint8_t)raw_zero->valuedouble;
                }
            }
...
{
    pp_od_yolo_v5_uu_params_t *pp = (pp_od_yolo_v5_uu_params_t *)pp_params;
    if (!pp) {
        return AI_OD_POSTPROCESS_ERROR_NO;
    }

    od_yolov5_pp_static_param_t *params = &pp->core;
...
static void od_pp_out_t_to_pp_result_t(od_pp_out_t *pObjDetOutput,
                                       pp_result_t *result,
                                       const pp_od_yolo_v5_uu_params_t *pp)
{
...
static int32_t run(void *pInput[], uint32_t nb_input, void *pResult, void *pp_params, void *nn_inst)
{
    ...
    // Use uint8 processing function for uint8 models
    error = od_yolov5_pp_process_uint8(&pp_input, &od_pp_out, params);
...
{
    pp_od_yolo_v5_uu_params_t *pp = (pp_od_yolo_v5_uu_params_t *)params;
...
const pp_entry_t pp_entry_od_yolo_v5_uu = {
    .name = "pp_od_yolo_v5_uu",
    .vt = &vt
};
