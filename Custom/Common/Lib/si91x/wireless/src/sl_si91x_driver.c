...
#else
  uint32_t events = sli_si91x_wait_for_event(NCP_HOST_COMMON_RESPONSE_EVENT, 15000);
  if (!(events & NCP_HOST_COMMON_RESPONSE_EVENT)) {
...
sl_status_t sl_si91x_driver_deinit(void)
{
  ...
  // TODO: When the WiFi chip malfunctions and cannot communicate, there is a probability of failure on this side
